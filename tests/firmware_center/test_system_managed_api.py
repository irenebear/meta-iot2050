...
def test_default_rollback_path_is_shared_and_explicit_path_is_compatible(system_fwu):
    assert system_fwu.resolve_rollback_path() == system_fwu.DEFAULT_ROLLBACK_TAR
    assert system_fwu.resolve_rollback_path(system_fwu.DEFAULT_ROLLBACK_DIR) == system_fwu.DEFAULT_ROLLBACK_TAR
    assert system_fwu.resolve_rollback_path(["/custom/backup"]) == \
        "/custom/backup/.rollback_fw/rollback_backup_fw.tar"


def test_rollback_inspection_requires_valid_metadata(system_fwu, tmp_path):
    rollback = tmp_path / "rollback_backup_fw.tar"
    with tarfile.open(rollback, "w") as archive:
        add_bytes(archive, "firmware.bin", b"old-firmware")

    with pytest.raises(system_fwu.UpgradeError) as error:
        system_fwu.inspect_system_rollback(tmp_path)
    assert error.value.code == system_fwu.ErrorCode.ROLLBACK_FAILED.value
